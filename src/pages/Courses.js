        ...
        {paginatedCourses.length > 0 ? (
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 sm:gap-6 lg:gap-8 mb-10 sm:mb-12 lg:mb-16">
            {paginatedCourses.map((course) => (
              <div key={course.id} onClick={() => navigate(`/courses/${course.slug || course.id}`)} className="bg-white rounded-lg sm:rounded-xl overflow-hidden shadow-sm hover:shadow-lg transition duration-300 cursor-pointer">
                ...
                <div className="p-4 sm:p-5 lg:p-6 space-y-3 sm:space-y-4">
                  ...
                  <div className="flex gap-2 sm:gap-3 pt-3 sm:pt-4">
                    <button onClick={(e) => { e.stopPropagation(); navigate(`/courses/${course.slug || course.id}`); }} className="flex-1 bg-indigo-600 text-white py-2 sm:py-2.5 rounded-lg text-xs sm:text-sm font-medium hover:bg-indigo-700 transition">
                      {d.card.enrollBtn}
                    </button>
                    <button onClick={(e) => { e.stopPropagation(); navigate(`/courses/${course.slug || course.id}`); }} className="flex-1 border border-gray-300 py-2 sm:py-2.5 rounded-lg text-xs sm:text-sm font-medium hover:bg-gray-50 transition">
                      {d.card.detailsBtn}
                    </button>
                  </div>
                </div>
      ...
      "hasOfferCatalog": {
        ...
        "itemListElement": allCourses.map((c) => ({
          ...
          "itemOffered": {
            ...
            "url": `https://trivoxatechnologies.in/courses/${c.slug || c.id}`
          }
        }))
      }
    },
