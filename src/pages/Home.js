            ...
            <div key={c.id} className="bg-white border rounded-2xl overflow-hidden shadow-sm hover:shadow-md transition">
              ...
              <div className="p-5">
                ...
                <div className="flex gap-3 mt-4">
                  <button onClick={() => navigate(`/courses/${c.slug || c.id}`)} className="flex-1 bg-indigo-600 text-white text-sm py-2 rounded-lg hover:bg-indigo-700 transition">{d.courses.enrollBtn}</button>
                  <button onClick={() => navigate(`/courses/${c.slug || c.id}`)} className="flex-1 border text-sm py-2 rounded-lg hover:bg-gray-50 transition">{d.courses.syllabusBtn}</button>
                </div>
              </div>
            </div>
          ))}
...
export default function Home() {
  ...
  usePageMeta(d.seo || {});
  useSchema([
    { "@context": "https://schema.org", "@type": "ItemList", "name": d.courses.heading, "itemListElement": d.courses.items.map((c, i) => ({ "@type": "ListItem", "position": i + 1, "name": c.title, "url": `https://trivoxatechnologies.in/courses/${c.slug || c.id}` })) },
